...
body::before {
  ...
  background:
    repeating-linear-gradient(0deg, transparent, transparent 39px, rgba(0,210,255,0.018) 39px, rgba(0,210,255,0.018) 40px),
...
.header-tag {
  ...
  color: var(--accent); background: rgba(0,210,255,0.08); border: 1px solid rgba(0,210,255,0.2);
...
.drop-sub code {
  background: #151515; border-radius: 3px; padding: 1px 6px;
  font-family: 'IBM Plex Mono', monospace; font-size: 0.7rem; color: #aaa;
}
...
select {
  ...
  font-family: 'Manrope', sans-serif; font-size: 0.8rem; padding: 7px 12px;
...
.btn-generate {
  ...
  font-family: 'Manrope', sans-serif; font-weight: 800; font-size: 0.8rem;
...
.kpi-val {
  ...
  font-weight: 700; color: var(--accent); display: block; margin-bottom: 4px;
}
...
.p2 { background: #FF8C00; }
.p3 { background: var(--gold); }
.p4 { background: var(--accent); }
.p5 { background: var(--green); }
.p6 { background: var(--muted2); }
...
.loading {
  ...
  font-family: 'IBM Plex Mono', monospace; color: var(--muted2); font-size: 0.8rem;
}
.spinner {
...
footer {
  ...
  font-size: 0.65rem; color: var(--muted); text-align: center; line-height: 1.8;
}
...
.ai-banner {
  ...
  border-radius: 8px; padding: 12px 16px; margin-bottom: 20px; font-size: 0.75rem;
}
...
.ai-chip {
  ...
  padding: 2px 8px; font-family: 'IBM Plex Mono', monospace; font-size: 0.62rem; color: var(--muted2);
}
.ai-chip span { color: var(--accent); }
.api-row {
  display: flex; align-items: center; gap: 10px; margin-bottom: 20px;
  padding: 14px 16px; background: var(--bg2); border: 1px solid var(--border); border-radius: 8px;
}
...
.api-row input[type=password] {
  ...
  font-family: 'IBM Plex Mono', monospace; font-size: 0.75rem; padding: 7px 12px; border-radius: 4px; outline: none;
}
...
.cal-month {
  background: var(--card); border: 1px solid var(--border);
  border-radius: 8px; padding: 16px; min-width: 224px;
}
.cal-month-title {
  font-family: 'IBM Plex Mono', monospace; font-size: 0.7rem; font-weight: 700;
...
@media print {
  ...
  .drop-zone, .controls, .btn-export, .btn-generate, .api-row, footer { display: none !important; }
...
function renderAnalytics() {
  ...
  const natures = [...new Set(parsedData.map(r => r.nature))].length;
  ...
  priorityChart = new Chart(document.getElementById('priorityChart'), {
    ...
    options: {
      ...
      plugins: { legend: { position: 'right', labels: { color: '#888', font: { size: 10 }, boxWidth: 10, padding: 8 } } }
    }
  });
...
function findOptimalShifts(scores, shiftLen, numShifts) {
  ...
  // Pick evenly-spaced optimal shifts to cover 24 hours
  const step = Math.floor(24 / numShifts);
  const result = [];
  for (let i = 0; i < numShifts; i++) {
    const base = (i * step) % 24;
    // Find best shift starting within ±2 hours of base
...
function renderCoverageChart(hourScore, shifts) {
  ...
  // Color each bar by which shift covers it
...
async function detectColumnsWithAI(headerRow, sampleRows) {
  ...
  const prompt = `You are analyzing a CSV file from a law enforcement CAD (Computer-Aided Dispatch) system.
...
Return ONLY valid JSON, no markdown, no explanation:
{
  ...
  "notes": "<brief one-sentence description of what agency/system this looks like>"
}`;
...
function showAIBanner(mapping, header) {
  ...
  const chips = Object.entries(fieldLabels)
    .filter(([k]) => mapping[k] >= 0)
    .map(([k, label]) => `<div class="ai-chip">${label} → <span>${header[mapping[k]] || '?'}</span></div>`)
    .join('');
...
// ─────────────────────────────────────────────
// OVERRIDE parseCSV TO USE AI WHEN KEY PRESENT
// ─────────────────────────────────────────────
...
function parseCSVWithMapping(text, filename, aiMapping) {
  ...
  if (aiMapping) {
    iDate     = aiMapping.date     ?? -1;
    iNature   = aiMapping.nature   ?? -1;
    iPriority = aiMapping.priority ?? -1;
    iX        = aiMapping.x        ?? -1;
    iY        = aiMapping.y        ?? -1;
    iAddress  = aiMapping.address  ?? -1;
    iCallNum  = aiMapping.callnum  ?? -1;
  } else {
    const col = name => {
      const variants = {
  ...
  for (let i=1; i<lines.length; i++) {
    ...
    records.push({ hour, priority, nature, lat, lon,
      address:get(iAddress), dateStr:get(iDate), callNum, dt });
  }
...
function renderCalendar() {
  ...
  // Build date → { score per shift } map
  const dayMap = {};
  parsedData.forEach(r => {
    if (!r.dt) return;
  ...
  // Group by year-month, skip empty months
  const monthMap = {};
...
// Override generate to also refresh calendar
...
function generate() {
  ...
  document.getElementById('schedTitle').textContent =
    `Recommended ${shiftHours}-Hour Shift Schedule (${numShifts} shifts · ${opsCount} operator${opsCount>1?'s':''}/shift)`;
...
function priorityWeight(p) {
  const w = {1:10, 2:7, 3:5, 4:3, 5:2, 6:1, 7:1};
  return w[p] || 1;
}
...
if app_mode == "📅 Operator Scheduler":
    st.title("📅 DFR Operator Schedule Optimizer")
    st.markdown("Use this standalone tool to analyze historical call volume and generate optimal staffing shifts.")
